...
useAngle = False     # if True, thifts highst col to ringfinger
...
caseWidth  = 12 *spacing + 2*wallWidth  # absolute case width
...
def getRowPos(rowOffset=0):
    ...
    points =  [ 
         ( 0.5 * spacing, topSwitchY - 3 * colStagger ),
         ( 1.5 * spacing, topSwitchY - 2 * colStagger ),
         ( 2.5 * spacing, topSwitchY - 1 * colStagger ),
         ( 3.5 * spacing, topSwitchY - 0 * colStagger ),
         ( 4.5 * spacing, topSwitchY - 1 * colStagger ),
         ( 5.5 * spacing, topSwitchY - 2 * colStagger )
    ]
...
#     kb = kb.add(full, name="kb", color=cq.Color(60/255,120/255,180/255))
...
# exporters.exportDXF(full.faces("<Z"), "bottom.dxf")
